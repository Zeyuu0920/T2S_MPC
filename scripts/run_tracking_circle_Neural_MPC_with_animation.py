...
def draw_overlay(frame, disturbance, current_time, method_name="Neural MPC"):
    h, w = frame.shape[:2]
...
for run_id in range(NUM_RUNS):
    ...
    if RECORD_DEMO and run_id == DEMO_RUN_ID:
        ...
        video_name_parts = [f"Neural_MPC_tracking_circle_demo_{WIND_TYPE}"]
        if args.A is not None:
    ...
    for i in range(Steps):
        ...
        x_ref_now = center_x + radius * np.cos(omega * current_time)
        z_ref_now = center_z + radius * np.sin(omega * current_time)
        ...
        next_time = (i + 1) * dt
        x_ref_next = center_x + radius * np.cos(omega * next_time)
        z_ref_next = center_z + radius * np.sin(omega * next_time)
