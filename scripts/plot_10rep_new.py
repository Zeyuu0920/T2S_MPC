...
variants = {
    "Nominal MPC": "Nominal_MPC_periodic_A0.003_period2_noise0.0005_seed",
    "Neural MPC": "Neural_MPC_periodic_A0.003_period2_noise0.0005_seed",
    "T2S-MPC(w/o time emd)": "T2S_wo_time_emd_periodic_A0.003_period2_noise0.0005_seed",
    "T2S-MPC(w/o 2 scales)": "T2S_wo_two_scale_periodic_A0.003_period2_noise0.0005_seed",
    "T2S-MPC(ours)": "T2S_MPC_periodic_A0.003_period2_noise0.0005_seed",

}

# 这里改成 linear，和 base_dir 保持一致
...
if len(inset_curves) > 0:
    ...
    if len(ymins) > 0 and len(ymaxs) > 0:
        y1 = max(0.0, min(ymins))   # error 不应为负，所以下限截到 0
        y2 = max(ymaxs)
